...
const TIPOS = [
  'Constancia ART Empleados',
  'Seguro de Vida Colectivo Obligatorio (SVCO)',
  'Plano de Obra',
  'Permiso Municipal',
  'Certificado IERIC Inicio de Obra',
  'Seguro de Caución',
  'Libro de Obra',
  'Acta de Inicio',
...
export default function DocumentosObraTab({
  idObra,
  documentosIniciales,
}: {
  idObra: number
  ...
  async function guardar() {
    ...
    try {
      const path = `obras/${idObra}/${Date.now()}_${archivo.name}`
      const { error: uploadError } = await supabase.storage.from('documentos').upload(path, archivo)
      if (uploadError) throw uploadError

      const { data, error: dbError } = await supabase.from('obra_documentos').insert({
        id_obra: idObra,
        tipo: tipoFinal,
              ...
              {tipo === '__custom__' && (
                <div>
                  ...
                  <input value={tipoCustom} onChange={e => setTipoCustom(e.target.value)}
                    style={inputStyle} placeholder="Ej: Certificado de Ensayo" />
                </div>
              )}
              <div>
                <label style={labelStyle}>Descripción (opcional)</label>
                <input value={descripcion} onChange={e => setDescripcion(e.target.value)}
                  style={inputStyle} placeholder="Ej: Vigente hasta dic 2026" />
              </div>
              <div>
