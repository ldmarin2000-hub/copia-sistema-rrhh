...
export default function ModalPermisos({ usuario, empresas, roles, permisos: permisosIniciales, obras, usuarioObras: uObrasIniciales, onCerrar }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div style={{ background: '#161b22', border: '0.5px solid #30363d', borderRadius: '10px', width: '100%', maxWidth: '520px', padding: '24px', maxHeight: '90vh', overflowY: 'auto' }}>

        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            <h2 style={{ fontSize: '16px', fontWeight: 500, color: '#e6edf3', margin: '0 0 2px' }}>Permisos de {usuario.nombre}</h2>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          {permisos.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {permisos.map((p) => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#0d1117', border: '0.5px solid #30363d', borderRadius: '6px', padding: '8px 12px' }}>
                  <div>
                    <span style={{ fontSize: '13px', color: '#e6edf3' }}>{p.empresas.razon_social}</span>
                    <span style={{ marginLeft: '8px', background: '#1a2a3a', color: '#58a6ff', fontSize: '11px', padding: '2px 6px', borderRadius: '4px' }}>{p.roles.descripcion}</span>
                  </div>
                  <button onClick={() => quitar(p.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#f85149' }}><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Agregar permiso */}
        <div style={{ borderTop: '0.5px solid #30363d', paddingTop: '16px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
            <button onClick={onCerrar} style={{ background: 'transparent', border: '0.5px solid #30363d', color: '#8b949e', borderRadius: '6px', padding: '7px 16px', fontSize: '13px', cursor: 'pointer' }}>Cerrar</button>
            <button onClick={agregar} disabled={loading || !idEmpresa || !idRol} style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', padding: '7px 16px', fontSize: '13px', cursor: 'pointer', opacity: loading ? 0.6 : 1 }}>
        ...
        {(esJefeObra || uObras.length > 0) && (
          <div style={{ borderTop: '0.5px solid #30363d', paddingTop: '16px' }}>
            <p style={{ fontSize: '12px', color: '#8b949e', marginBottom: '10px' }}>Obras asignadas (Jefe de Obra)</p>

            {uObras.length === 0 ? (
              <p style={{ fontSize: '13px', color: '#484f58', marginBottom: '10px' }}>Sin obras asignadas</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '12px' }}>
                {uObras.map(uo => (
                  <div key={uo.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#0d1117', border: '0.5px solid #30363d', borderRadius: '6px', padding: '7px 12px' }}>
                    <span style={{ fontSize: '13px', color: '#e6edf3' }}>{(uo as any).obras?.nombre || `Obra #${uo.id_obra}`}</span>
                    <button onClick={() => quitarObra(uo.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#f85149' }}><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            )}
            ...
            {obrasDisponibles.length > 0 && (
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={agregarObra} disabled={loadingObra || !idObraAdd} style={{ background: '#1a2a3a', border: '0.5px solid #58a6ff40', color: '#58a6ff', borderRadius: '6px', padding: '7px 14px', fontSize: '13px', cursor: 'pointer', whiteSpace: 'nowrap', opacity: loadingObra ? 0.6 : 1 }}>
                  + Agregar
                </button>
              </div>
            )}
